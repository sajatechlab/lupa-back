...
export class TableDownloadService {
  ...
  async downloadAndProcessZip(
    ...
    try {
      ...
      // Step 2: Process all entries in parallel
  ...
  private async parseAndSaveXml(
    ...
    try {
      ...
      const result = parserResult.Invoice;
      ...
      // Fetch or create company and third party in parallel
  ...
  //   try {
  //     // Try to insert invoice, ignore if it already exists
  //     const insertResult = await this.invoiceRepository
  //       .createQueryBuilder()
  //       .insert()
  //       .into('invoice')
  //       .values(invoiceData)
  //       .orIgnore() // This will skip if invoice exists
  //       .execute();

  //     // Only create invoice lines if we actually inserted a new invoice
